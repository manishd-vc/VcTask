...
export default function PartnerAssessmentAddQuestions({ open, onClose, refetch }) {
  const { id } = useParams();
  ...
  const { partnerAssessmentQuestions, predefineAssessmentQuestions } = useSelector((state) => state?.partner);

  const selectedPredefinedIndexes =
    partnerAssessmentQuestions?.questions
      ?.map((q) => predefineAssessmentQuestions.findIndex((pq) => pq.queReference === q.queReference))
      ?.filter((i) => i !== -1) || [];
  ...
  const { mutate, isLoading } = useMutation(
    'createPartnerAssessmentQuestions',
    partnerApi.createPartnerAssessmentQuestions,
    {
      onSuccess: (response) => {
  ...
  const initialValues = {
    questions:
      partnerAssessmentQuestions?.questions?.filter(
        (q) => !predefineAssessmentQuestions.some((pq) => pq.queReference === q.queReference)
      ) || [],
  ...
  const handleSubmit = (values) => {
    const selectedPredefined = values?.selectedQuestions?.map((index) => predefineAssessmentQuestions[index]) || [];
    const payload = {
      ...
      >
        {({ values, setFieldValue, dirty }) => (
          <Form>
            <DialogContent>
              <PredefineQuestion
                title="Choose Questions to add in assessment"
                predefineData={predefineAssessmentQuestions}
                expanded={expanded}
              ...
              <CustomQuestion
                qType={qType}
                predefinedQuestionLength={predefineAssessmentQuestions?.length}
                module="partner"
              />
            </DialogContent>
            <DialogActions>
