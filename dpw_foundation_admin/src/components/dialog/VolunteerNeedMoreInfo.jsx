...
export default function VolunteerNeedMoreInfo({ open, onClose, data, backTo }) {
  ...
  const { mutate: deleteMediaMutation, isLoading: isDeleteLoading } = useMutation(
    'deleteMedia',
    volunteerApi.deleteNeedMoreInfoDocuments,
    {
      onSuccess: (response) => {
  ...
  const { mutate, isLoading, isSuccess } = useMutation(volunteerApi.uploadNeedMoreInfoDocuments, {
    onSuccess: (response) => {
  ...
  const { mutate: volunteerCommonStatusUpdate, isLoading: isVolunteerCommonStatusUpdateLoading } = useMutation(
    volunteerApi.volunteerCommonStatusUpdate,
    {
      onSuccess: async (response) => {
        router.push(backTo);
        onClose();
        dispatch(setToastMessage({ message: response?.message, variant: 'warning', title: 'Need more Information' }));
      },
      onError: (err) => {
    ...
    <Dialog aria-label="Upload-documents" onClose={onClose} open={open} maxWidth={'md'}>
      ...
      <Formik
        ...
        onSubmit={(values) => {
          ...
          volunteerCommonStatusUpdate({ id: data?.id, payload });
        }}
      >
            ...
            <Form id="needMoreInfoForm">
                          ...
                          <Typography component="div" variant="body2" color="text.secondarydark">
                            ...
                            <Tooltip title="Remove" arrow>
                              <IconButton
                                ...
                                onClick={() => {
                                  if (fileID) {
                                    deleteMediaMutation({ entityId: data?.id });
                                  }
                                }}
              ...
              <DialogActions>
                ...
                <LoadingButton
                  variant="contained"
                  color="primary"
                  form="needMoreInfoForm" // The form to be submitted
                  onClick={handleSubmit}
                  loading={isVolunteerCommonStatusUpdateLoading}
                  disabled={!dirty}
                >
                  Send
                </LoadingButton>
