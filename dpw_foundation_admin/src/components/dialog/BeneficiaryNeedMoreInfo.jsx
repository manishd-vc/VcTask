...
export default function BeneficiaryNeedMoreInfo({ open, onClose, stageType, data, backTo }) {
  ...
  const { mutate: deleteMediaMutation, isLoading: isDeleteLoading } = useMutation(
    'deleteMedia',
    beneficiaryApi.deleteInKindNeedMoreInfo,
    {
      onSuccess: (response) => {
  ...
  const { mutate, isLoading, isSuccess } = useMutation(beneficiaryApi.uploadInKindNeedMoreInfo, {
    onSuccess: (response) => {
  ...
  const { mutate: beneficiaryCommonStatusUpdate, isLoading: isBeneficiaryCommonStatusUpdateLoading } = useMutation(
    beneficiaryApi.updateInKindContributionStatus,
    {
    ...
    <Dialog aria-label="Upload-documents" onClose={onClose} open={open} maxWidth={'md'}>
      ...
      <Formik
        ...
        onSubmit={(values) => {
          ...
          beneficiaryCommonStatusUpdate({ entityId: data?.id, payload });
        }}
      >
            ...
            <Form id="needMoreInfoForm">
                    ...
                    <Stack flexDirection="row" flexWrap="wrap" alignItems="center" gap={1.2}>
                      <FileUpload
                        ...
                        onChange={(event) => {
                          ...
                          mutate({ entityId: data?.id, payload: formData });
                        }}
                        disabled={isLoading}
                        size="small"
                      />
                          ...
                          <Typography component="div" variant="body2" color="text.secondarydark">
                            ...
                            <Tooltip title="Remove" arrow>
                              <IconButton
                                ...
                                onClick={() => {
                                  if (fileID) {
                                    deleteMediaMutation({ entityId: data?.id, type: stageType });
                                  }
                                }}
              ...
              <DialogActions>
                ...
                <LoadingButton
                  ...
                  loading={isBeneficiaryCommonStatusUpdateLoading}
                  disabled={!dirty}
                >
                  Send
                </LoadingButton>
