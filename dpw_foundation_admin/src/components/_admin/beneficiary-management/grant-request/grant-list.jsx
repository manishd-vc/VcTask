...
import BeneficiariegrantRequestsRow from 'src/components/table/rows/beneficiariegrantRequestsRow';
...
export default function GrantList() {
  ...
  const { isLoading, isFetching, refetch } = useQuery(
    ...
    () =>
      beneficiaryApi.beneficiaryGrantPagination({
        userId: userId,
        page: +pageParam,
        rows: +rowsParam,
        sort: '',
  ...
  const { mutate: exportMutate } = useMutation(
    'export-beneficiary-grant-admin',
    beneficiaryApi.exportAllGrantRequests,
    {
      onSuccess: (data) => {
  ...
  return (
    <Table
      ...
      row={BeneficiariegrantRequestsRow}
