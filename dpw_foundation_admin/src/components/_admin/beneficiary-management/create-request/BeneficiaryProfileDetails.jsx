...
export default function BeneficiaryProfileDetails() {
  ...
  const getLabel = (fieldKey) => {
    const isIndividual = beneficiaryUserDetails?.accountType === 'Individual';
    return donorLabels[fieldKey]?.[isIndividual ? 'individual' : 'organization'] || '';
  };
  const isOrganization = useMemo(
    () => beneficiaryUserDetails?.accountType === 'Organization',
    [beneficiaryUserDetails?.accountType]
  );

  const isOrganizationName = useMemo(
    () => beneficiaryUserDetails?.organizationName,
    [beneficiaryUserDetails?.organizationName]
  );
  const isOrganizationRegistrationNumber = useMemo(
    () => beneficiaryUserDetails?.organizationRegistrationNumber,
    [beneficiaryUserDetails?.organizationRegistrationNumber]
  );
  ...
  return (
    <>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} sm={6}>
          <Stack spacing={0.5}>
            ...
            <Typography variant="subtitle4" color="text.secondarydark">
              {beneficiaryUserDetails?.firstName || ''}
            </Typography>
          </Stack>
        </Grid>
        <Grid item xs={12} sm={6}>
          <Stack spacing={0.5}>
            ...
            <Typography variant="subtitle4" color="text.secondarydark">
              {beneficiaryUserDetails?.lastName || ''}
            </Typography>
          </Stack>
        </Grid>
        <Grid item xs={12} sm={6}>
          ...
          <>
            <Grid item xs={12} sm={6}>
              {isOrganizationName ? (
                ...
                <FieldWithSkeleton
                  isLoading={beneficiaryUserDetailsLoading}
        ...
        <Grid item xs={12} sm={6}>
          {beneficiaryUserDetails?.countryName ? (
            <Stack spacing={0.5}>
              <Typography variant="body3" color="text.secondary">
                {getLabel('country')}
              </Typography>
        ...
        <Grid item xs={12} sm={6}>
          {beneficiaryUserDetails?.stateName ? (
            <Stack spacing={0.5}>
              <Typography variant="body3" color="text.secondary">
                {getLabel('state')}
              </Typography>
            ...
            <FieldWithSkeleton isLoading={beneficiaryUserDetailsLoading} error={touched.state && errors.state}>
              <TextFieldSelect
                id="state"
                label={
                  <>
        ...
        <Grid item xs={12} sm={6}>
          {beneficiaryUserDetails?.city ? (
            ...
            <FieldWithSkeleton isLoading={beneficiaryUserDetailsLoading} error={touched.city && errors.city}>
              <TextFieldSelect
                id="city"
                label={
                  <>
        ...
        <Grid item xs={12} sm={6}>
          {beneficiaryUserDetails?.mailingAddress ? (
            <Stack spacing={0.5}>
              <Typography variant="body3" color="text.secondary">
                Mailing Address
              </Typography>
      ...
      <Box sx={{ my: 3 }}>
        <DocumentSection entityId={beneficiaryUserDetails?.userId} />
      </Box>
      <BankingInformation data={beneficiaryUserDetails} dataLoading={beneficiaryUserDetailsLoading} />
    </>
  );
}
