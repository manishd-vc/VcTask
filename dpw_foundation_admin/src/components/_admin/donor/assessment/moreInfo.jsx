 ...
 */
const MoreInfo = ({ open, onClose, onSubmit, isLoading, id, type }) => {
  ...
  const renderTypeStatus = () => {
    switch (type) {
      case 'admin':
        return 'ASSESSMENT_MORE_INFO_REQUIRED';
      case 'assessment':
        return 'ASSESSMENT_MORE_INFO_REQUIRED';
      case 'hod':
        return 'DOCUMENT_MORE_INFO_REQUIRED';
      default:
        return 'ASSESSMENT_MORE_INFO_REQUIRED';
    }
  };
  ...
  const formik = useFormik({
    initialValues: {
      content: '', // Field for "more info" text
      fileId: '' // Field for the uploaded file ID
    },
    validationSchema: Yup.object({
      content: Yup.string().required('More info is required') // Validation rule for "more info"
    }),
    onSubmit: (values) => {
  ...
  const handleAttachmentsUpload = async (file) => {
    const result = await api.uploadNeedMoreInfoFilesForAssement(id, file, type); // Call the API to upload the file
    if (result.status == 200) {
      dispatch(setToastMessage({ message: result?.message, variant: 'success' }));
      setUploadedFile(result.data); // Set the uploaded file ID on success
    } else {
