...
FileUpload.propTypes = {
  ...
  // 'accept' is a string that specifies the accepted file types
...
export default function FileUpload({
  ...
  const acceptTypes = validationFiles?.values?.find((item) => item.code === 'frontendFileType')?.label || '';
  ...
  return (
    <Button type="button" variant="contained" size={size} component="label" startIcon={icon} disabled={disabled}>
      ...
      <input
        ...
        onChange={onChange}
        accept={accept || acceptTypes} // Uses either the passed `accept` prop or the one derived from the master data
      />
    </Button>
  );
}
