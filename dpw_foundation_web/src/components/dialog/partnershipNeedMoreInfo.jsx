...
export default function PartnershipNeedMoreInfo({ open, onClose, stageType, data, backTo }) {
  ...
  const { mutate: deleteMediaMutation, isLoading: isDeleteLoading } = useMutation(
    'deleteMedia',
    partnerApi.deleteNeedMoreInfoDocuments,
    {
      onSuccess: (response) => {
  ...
  const { mutate, isLoading, isSuccess } = useMutation(partnerApi.uploadNeedMoreInfoDocuments, {
    onSuccess: (response) => {
      dispatch(setToastMessage({ message: response.message, variant: 'success' }));
  ...
  const { mutate: partnershipCommonStatusUpdate, isLoading: isPartnershipCommonStatusUpdateLoading } = useMutation(
    partnerApi.partnershipCommonStatusUpdate,
    {
