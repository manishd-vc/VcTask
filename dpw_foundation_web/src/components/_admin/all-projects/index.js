...
export default function AllProjectsListing() {
  ...
  const charitableProjectStatus = getLabelObject(masterData, 'dpwf_contribution_project_statusfix: status: ');
  ...
  useEffect(() => {
    if (charitableProjectStatus) {
      const statusData = [];
      for (const data of charitableProjectStatus?.values) {
        statusData.push({
          id: data?.code,
          title: data?.label
        });
  ...
  }, [charitableProjectStatus]);

  return (
    <>
      <HeaderBreadcrumbs admin heading="All Projects" />
      <Table
        headData={TABLE_HEAD}
        data={tableRows}
        isLoading={isLoading}
