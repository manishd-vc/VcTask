...
import { BlurImage, Image } from 'src/components/blurImage';
// icons
        ...
        <Box p={3}>
          <Grid container spacing={2} sx={{ img: { borderRadius: '8px' } }}>
            {review.images.map((image) => (
              <Grid item xs={6} md={3} lg={2} key={`data-key_${new Date().getTime()}`}>
                <Box
                  sx={{
                    position: 'relative',
                    height: 100
...
export default function ProductDetailsReviewList({ ...props }) {
  ...
  return (
    <Box>
      <List disablePadding>
        {reviews?.map((review) => (
          <ReviewItem key={`data-key_${new Date().getTime()}`} review={review} isLoading={isLoading} />
        ))}
      </List>
    </Box>
  );
